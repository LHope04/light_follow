...
class Follower(Node):
    def __init__(self):
        ...
        # 核心修改：增加 standalone 参数以支持独立巡线
        self.declare_parameter('standalone', False)
        self.standalone = self.get_parameter('standalone').value
        
        # 如果是独立模式，直接发布到 /cmd_vel 控制底盘；否则发给协同节点
        cmd_topic = '/cmd_vel' if self.standalone else '/line_follow/cmd_vel'
    ...
    def image_callback(self, msg):
        ...
        # 1. 严格 4 向哨兵检测
        top_zone = mask[0 : int(h*0.35), int(w*0.4) : int(w*0.6)]
        bot_zone = mask[int(h*0.65) : h, int(w*0.4) : int(w*0.6)]
        left_zone = mask[int(h*0.35) : int(h*0.65), 0 : int(w*0.25)]
        right_zone = mask[int(h*0.35) : int(h*0.65), int(w*0.75) : w]
